...
Aqui também se declara a **localidade** cujo calendário determina quais feriados incidem na contagem de prazos do certame, por referência de cidade (`UNI-REQ-0111`). O servidor não lhe atribui valor por omissão nem a deduz da unidade administradora, do campus ou da cidade de oferta: quem configura informa, e é obrigatória já na criação do processo — o cadastro nasce junto com o edital de abertura, que é onde a localidade regente está declarada. A interface pode apresentar o campo preenchido com a cidade da unidade administradora, que é a localidade regente na quase totalidade dos certames; o operador confirma ou troca antes de submeter. O **fuso horário** em que cada dia civil começa e termina não é perguntado — é o mesmo em todo certame da instituição, então o sistema o aplica e o congela na versão publicada.
