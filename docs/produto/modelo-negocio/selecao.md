...
- Uma retificação publicada no meio do caminho não retroage sobre recurso já em curso: o par de suspensividade, a irreversibilidade e o prazo de interposição que governam o recurso são os vigentes quando o direito de recorrer nasceu — a publicação do ato recorrido —, nunca os vigentes na interposição ou no julgamento. Um recurso contra um ato publicado já sob a regra retificada é regido pela regra nova; não é congelamento perpétuo do processo, é imunidade retroativa por ato (`UNI-REQ-0093`). Quando a retificação deixa de configurar uma fase para aceitar recurso, uma interposição contra ato publicado já sob essa configuração é recusada com erro nomeado, nunca aceita silenciosamente sem bloqueio (`UNI-REQ-0094`).
- O que já precisa ser configurado e congelado na entrega atual. Embora o processamento do recurso seja etapa futura, a informação que o alimenta é configurada e congelada desde a publicação: para cada fase configurada para aceitar recurso e cada instância, o edital congela o tempo de bloqueio (um par valor mais unidade, ou sem bloqueio quando não se aplica), a marca de que o ato daquela fase é irreversível e o prazo de interposição. Sem esses campos congelados, o mecanismo futuro não teria como saber o que travar nem reconstruir a garantia de não retroatividade para processo publicado antes de o mecanismo existir — por isso uma fase configurada para aceitar recurso, mas publicada sem eles, é recusada; uma fase que não aceita recurso simplesmente não os tem. Configuração na entrega atual.
