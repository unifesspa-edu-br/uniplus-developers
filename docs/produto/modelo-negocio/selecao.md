...
- Há dois prazos distintos: o prazo para o candidato recorrer (contado da publicação do ato) e o tempo de bloqueio (que começa quando um recurso é efetivamente interposto). O bloqueio encerra no julgamento ou no fim do prazo, o que vier primeiro — um recurso nunca julgado não pode travar o certame indefinidamente.
- O alcance do bloqueio depende do objeto do recurso: um recurso de efeito coletivo trava o processo inteiro; um recurso individual trava apenas a parte afetada (curso, modalidade ou a vaga em disputa).
- Uma retificação publicada no meio do caminho não retroage sobre recurso já em curso: a regra que governa o recurso (prazo, ancoragem, bloqueio) é a vigente quando o direito de recorrer nasceu — a publicação do ato recorrido —, nunca a vigente na interposição ou no julgamento. Um recurso contra um ato publicado já sob a regra retificada é regido pela regra nova; não é congelamento perpétuo do processo, é imunidade retroativa por ato (`UNI-REQ-0093`). Quando a retificação remove a configuração de recurso de uma fase, uma interposição contra ato publicado já sob essa configuração é recusada com erro nomeado, nunca aceita silenciosamente sem bloqueio (`UNI-REQ-0094`).
- O que já precisa ser configurado e congelado na entrega atual. Embora o processamento do recurso seja etapa futura, a informação que o alimenta é configurada e congelada desde a publicação: para cada fase e cada instância, o edital congela o tempo de bloqueio (um par valor mais unidade, ou sem bloqueio quando não se aplica), a marca de que o ato daquela fase é irreversível, o prazo de interposição, a regra de ancoragem do ato recorrido e o escopo do bloqueio. Sem esses campos congelados, o mecanismo futuro não teria como saber o que travar nem reconstruir a garantia de não retroatividade para processo publicado antes de o mecanismo existir — por isso um edital publicado sem eles é recusado. Configuração na entrega atual.
