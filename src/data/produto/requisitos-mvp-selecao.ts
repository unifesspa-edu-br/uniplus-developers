...
export const requisitosMvpSelecao: Requisito[] = [
  ...
  },
  {
    ...
    enunciado:
      'O sistema deve manter os tipos de processo seletivo em cadastro institucional no módulo Configuração, com código canônico imutável e não reutilizável. Somente tipo ativo pode ser vinculado a novo Processo Seletivo ou a obrigatoriedade legal específica; ProcessoSeletivo preserva uma cópia por valor do tipo e VersaoConfiguracao a congela no envelope publicado. A desativação é prospectiva, e não terminal: um tipo desativado pode ser reativado, voltando à listagem pública com o mesmo código, sem que a identidade se reabra nem que os certames e as versões já produzidos mudem. Edital permanece o ato/documento normativo, não o agregado configurável.',
    grupo: 'dados',
    ...
    criterios_aceite:
      'Plataforma-admin cria, altera, desativa e reativa tipos; a listagem pública mostra somente ativos, e a de manutenção, restrita a plataforma-admin, mostra também os desativados, para que o tipo a reativar possa ser encontrado; reativar um tipo já ativo é recusado; código é único e imutável inclusive após desativação e reativação; Codigo, Nome e Descricao recusam U+0000 com erro 422; a criação de Processo Seletivo exige tipo ativo pelo campo tipoProcessoOrigemId, sem aceitar o contrato legado tipo; os oito tipos legados são semeados com os mesmos códigos e identificadores UUIDv7 determinísticos, e cada acesso fornece uma cópia independente para o agregado; ProcessoSeletivo preserva a cópia por valor; o envelope 0.0.7 contém exatamente 24 blocos, incluindo tipoProcesso; alterações, desativação ou reativação posteriores não mudam o Processo Seletivo nem as Versões de Configuração já produzidas.',
    verificacao:
      'Testes de domínio, validators e API para U+0000; ciclo de desativação e reativação pela API, com o código preservado e o tipo de volta à listagem pública; recusa de reativar tipo já ativo; escrita concorrente sobre o mesmo tipo resolvida por token de concorrência otimista; prova dos oito UUIDv7 RFC 9562 únicos na carga inicial e nas constantes legadas; teste de persistência de dois Processos Seletivos do mesmo tipo no mesmo DbContext; migração dos tipos legados; criação com tipo ativo/inativo; contrato HTTP de tipoProcessoOrigemId; prova exaustiva dos 24 blocos e fixtures 0.0.7 do envelope.',
    pagina_developers: '/produto/requisitos/',
    owner: 'Equipe backend; Tech Lead',
  },
  {
    requisito_id: 'UNI-REQ-0139',
    titulo: 'Vocabulário fechado de fases canônicas e tipos de banca',
    enunciado:
      'O sistema deve manter, no módulo Configuração, dois vocabulários fechados e correlatos do ciclo do processo seletivo: as fases canônicas do seu ciclo de vida e os tipos de banca que nele atuam. Cada fase e cada banca referencia um código do respectivo domínio fechado, validado em guarda de domínio, na aplicação e em CHECK constraint de banco; os demais atributos permanecem administráveis via CRUD — em tipo de banca, nome, fase típica orientativa e descrição; em fase canônica, nome, descrição, dono típico, origem da data, produção e definitividade de resultado, coleta de inscrição, agrupamento de etapas, permissão de complementação documental e base legal. A fase típica de uma banca é informação orientativa em texto livre, sem vínculo estrutural (chave estrangeira) com o cadastro de fases.',
    grupo: 'dados',
    tipo: 'requisito_dados',
    nivel: 'requisito',
    parent_id: 'UNI-REQ-0006',
  ...
  },
  {
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'A regra governa dois relógios distintos, cada um sob o seu contrato. A janela de solicitação é período do cronograma medido em dias corridos: não é interposição de recurso, não fica sujeita ao contrato de unidades de UNI-REQ-0113 e não depende de calendário de dias úteis nem de algoritmo de contagem. A publicação é recusada quando a janela começa em instante diferente da abertura das inscrições, termina junto com elas ou depois delas, ou não alcança cinco dias corridos contados excluindo o dia da abertura, com o quinto dia completando-se ao fim do último instante do dia no fuso congelado. A prorrogação é recusada quando ocorre após o encerramento da etapa, quando reduz o prazo, quando viola a duração mínima ou quando alcança ou ultrapassa o fim das inscrições; a reabertura de janela encerrada é recusada em qualquer hipótese. O prazo de recurso contra o indeferimento, esse sim, é interposição: a sua unidade segue UNI-REQ-0113, a convenção de contagem segue UNI-REQ-0112, e a publicação da fase é recusada sem calendário vigente e algoritmo declarado, na forma de UNI-REQ-0116.',
    verificacao:
      'Testes de publicação sobre as fronteiras da janela — início coincidente com a abertura das inscrições, término anterior ao fim delas e duração mínima — e sobre as recusas de prorrogação e de reabertura. Recurso da fase verificado pelo mesmo caminho da regra geral de recurso.',
    pagina_developers: '/produto/requisitos/',
  ...
  },
  {
    ...
    verificacao:
      'Implementada no backend: a oferta declara regime de turno e a lista de turnos, e INTEGRAL deixou o domínio de turno. Cobrem o requisito os testes de unidade das invariantes de regime e de cardinalidade no agregado, os testes de contrato que recusam cada violação por código de erro próprio, os testes de persistência em que o banco recusa lista vazia, turno repetido e quantidade incompatível com o regime, e o teste que prova a ordem canônica na leitura mesmo quando a linha foi gravada fora de ordem. O cadastro no painel de Configuração acompanha a mudança.',
    pagina_developers: '/produto/requisitos/',
    owner: 'Equipe backend; Equipe frontend',
  },
  {
