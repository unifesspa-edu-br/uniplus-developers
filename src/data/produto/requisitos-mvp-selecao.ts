...
export const requisitosMvpSelecao: Requisito[] = [
  ...
  },
  {
    ...
    tipo_issue_recomendado: 'Feature',
    criterios_aceite:
      'A configuração de documentos exigidos está decomposta em requisitos filhos implementáveis (aplicabilidade, identidade, base legal, operadores/DNF, vocabulário de fatos, formatos e idade, árvore de satisfação, consequência, cardinalidade, repetição por entidade e snapshot conjunto); uma issue de entrega aponta o filho efetivamente alterado, e o fechamento do agregador deriva do fechamento dos filhos.',
  ...
  },
  {
    ...
    enunciado:
      'O gatilho da exigência documental é um DNF (OU de cláusulas, E de condições) sobre um conjunto fechado e simétrico de operadores — IGUAL/DIFERENTE, EM/NAO_EM, MAIOR_IGUAL/MENOR_IGUAL —, cada condição validada por operador×domínio×valor. A avaliação é ternária (verdadeiro/falso/indeterminado) e fail-closed: fato não resolvido produz INDETERMINADO, que se propaga em E e entre as cláusulas do DNF e mantém a exigência pendente, nunca a descarta silenciosamente. Fatos multivalorados de escopo-processo (MODALIDADE, CONDICAO_ATENDIMENTO) usam pertinência: IGUAL é pertencer, EM é interseção não vazia, e DIFERENTE/NAO_EM são os complementos correspondentes; EM [] é falso e NAO_EM [] é verdadeiro com fato resolvido. A negação só inverte com fato resolvido (DIFERENTE/NAO_EM não são !IGUAL/!EM sobre ausência). Uma condição não pode referenciar fato cujo ponto de resolução seja posterior à fase da exigência (gate de fase).',
    grupo: 'funcional',
    tipo: 'requisito_funcional',
    nivel: 'requisito',
    parent_id: 'UNI-REQ-0016',
