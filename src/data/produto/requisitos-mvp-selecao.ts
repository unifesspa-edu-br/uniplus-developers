...
export const requisitosMvpSelecao: Requisito[] = [
  ...
  },
  {
    ...
    titulo: 'Campos de suspensividade, irreversibilidade e ancoragem do recurso no snapshot congelado (RN13)',
    enunciado:
      'A configuração do processo congela, por instância de recurso (primeira e segunda) e por fase configurada para aceitar recurso, o efeito suspensivo como um par valor-unidade — cuja ausência (nulo) significa que a pendência não bloqueia — junto com a marca de irreversibilidade do ato produzido pela fase e o prazo de interposição. O escopo do bloqueio não é um desses campos: deriva do objeto de cada recurso (UNI-REQ-0081) — erga omnes trava o processo inteiro, individual trava só o alcance afetado —, uma regra estrutural que não é configurável por fase nem muda com retificação, por isso não precisa ser congelada. Esses campos entram no snapshot imutável na publicação e não podem ser adiados: o motor que os avalia vem depois (UNI-REQ-0081), mas a informação que ele exigirá precisa estar congelada desde já (RN08) — inclusive para processos publicados antes de o motor existir, já que a versão publicada é imutável e não há como completá-la depois (UNI-REQ-0093). Uma fase que não é configurada para aceitar recurso simplesmente não tem esses campos — isso não invalida a publicação (UNI-REQ-0094).',
    grupo: 'negocio',
    tipo: 'regra_negocio',
    nivel: 'regra',
    parent_id: 'UNI-REQ-0014',
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'O snapshot congelado carrega, por fase configurada para aceitar recurso e por instância, o par de suspensividade (valor e unidade, ou nulo para não bloquear), a irreversibilidade do ato e o prazo de interposição. Um processo com uma fase configurada para aceitar recurso mas publicado sem algum desses três campos é rejeitado, pois a versão é imutável e o gate futuro não teria como reconstruí-los; uma fase que não é configurada para aceitar recurso não precisa desses campos, e sua ausência nesse caso não é falha de publicação. A configuração é a mesma para Seleção e Ingresso; a diferença de bloqueio decorre só dos valores congelados, nunca de ramificação por módulo em código.',
    verificacao: 'Teste de contrato do snapshot (campos presentes e imutáveis após publicação para fase configurada com recurso; fase sem recurso publica sem eles) (planejado).',
    pagina_developers: '/produto/regras-negocio/',
    owner: 'Tech Lead',
  },
  {
  ...
  },
  {
    ...
    enunciado:
      'A regra de suspensividade (par valor-unidade, irreversibilidade, prazo de interposição — os campos congelados por UNI-REQ-0080) que rege um recurso é a que estava vigente quando o direito de recorrer nasceu — a publicação do ato recorrido —, nunca a vigente no instante em que o candidato interpõe, nem a vigente quando a banca julga. Uma retificação publicada depois do ato recorrido, mesmo antes da interposição se o prazo já estava aberto, não pode encurtar o prazo, afrouxar o bloqueio nem mudar a irreversibilidade de um recurso que venha a ser interposto contra aquele ato — equivalente ao princípio de irretroatividade: mudança de regra não retroage em prejuízo de quem já exercia o direito. O escopo do bloqueio não integra esta garantia: não é congelado por fase (UNI-REQ-0080), deriva do objeto de cada recurso e essa derivação não muda com retificação. Um recurso interposto contra um ato publicado sob a regra nova nasce, sim, sob a regra nova — não é congelamento perpétuo do processo inteiro, só imunidade retroativa por ato. Os campos que esta garantia precisa já são congelados desde o MVP por UNI-REQ-0080, justamente para que processos publicados antes deste motor existir não fiquem sem a garantia depois — este requisito cobre só o comportamento de resolução do motor de interposição (UNI-REQ-0081), não o congelamento em si, que não se aplica antes do motor existir. Base legal: dispositivo exato do prazo de interposição e do efeito suspensivo do recurso administrativo federal ainda não confirmado com validação jurídica — pendência a resolver antes de publicar processo real sob esta garantia, nunca citação aproximada.',
    grupo: 'negocio',
    tipo: 'regra_negocio',
    nivel: 'regra',
    parent_id: 'UNI-REQ-0048',
    ...
    tipo_issue_recomendado: 'None',
    criterios_aceite:
      'O par de suspensividade, a irreversibilidade e o prazo de interposição de um recurso são resolvidos, os três juntos, pelos campos já congelados por UNI-REQ-0080 na versão vigente quando o ato recorrido foi publicado, nunca pela vigente no instante da interposição nem no julgamento; uma retificação publicada entre a publicação do ato recorrido e a interposição não altera nenhum desses três campos para um recurso que venha a ser interposto contra aquele ato; um recurso contra um ato publicado já sob a regra retificada é regido pela regra nova, sem herdar a proteção de atos anteriores.',
    verificacao: 'Revisão futura.',
    pagina_developers: '/produto/regras-negocio/',
    owner: 'Tech Lead',
  },
