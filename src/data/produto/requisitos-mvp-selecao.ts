...
export const requisitosMvpSelecao: Requisito[] = [
  ...
  },
  {
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'O snapshot congelado carrega, por fase configurada para aceitar recurso, o prazo de interposição (valor, unidade e convenção de contagem, só dias corridos ou horas) referente só à interposição que o Uni+ conduz — nunca um prazo de interposição para a instância superior, que não tem essa etapa dentro do sistema (UNI-REQ-0081) e por isso não precisa desse campo. A mesma fase carrega também, por instância (primeira e segunda), o par de suspensividade (valor, unidade e convenção de contagem — corridos, horas ou úteis —, ou nulo para não bloquear); quando a convenção é dias úteis, o snapshot carrega também o identificador da versão do calendário e o identificador da versão do algoritmo de contagem vigentes na publicação, nenhum dos dois recalculado depois. Um processo com uma fase configurada para aceitar recurso mas publicado sem o prazo de interposição, sem convenção de contagem em qualquer um dos pares congelados, com valor zero ou negativo em qualquer um dos dois, ou com suspensividade em dias úteis sem identificador de calendário vigente e de algoritmo de contagem vigente (hoje sempre o caso, pois o algoritmo não existe como artefato versionável), é rejeitado, pois a versão é imutável e o gate futuro não teria como reconstruí-los; um prazo de interposição em dias úteis é sempre rejeitado; uma fase que não é configurada para aceitar recurso não precisa desses campos, e sua ausência nesse caso não é falha de publicação. Nem escopo do bloqueio nem irreversibilidade do ato entram no snapshot por fase: o primeiro é resolvido do objeto de cada recurso, o segundo do cadastro do tipo de ato — nenhum dos dois é configuração por edital. A configuração é a mesma para Seleção e Ingresso; a diferença de bloqueio decorre só dos valores congelados, nunca de ramificação por módulo em código.',
    verificacao: 'Teste de contrato do snapshot (prazo de interposição e par de suspensividade presentes e imutáveis após publicação para fase configurada com recurso, com convenção de contagem sempre explícita; valor zero ou negativo recusado nos dois; identificadores de calendário e de algoritmo de contagem congelados junto com suspensividade em dias úteis, imunes a correção posterior de qualquer um dos dois; dias úteis recusado no prazo de interposição sempre, e na suspensividade sempre que faltar calendário vigente ou algoritmo de contagem vigente identificável; instância superior publica sem prazo de interposição próprio; fase sem recurso publica sem eles) (planejado).',
    pagina_developers: '/produto/regras-negocio/',
    owner: 'Tech Lead',
  },
  {
