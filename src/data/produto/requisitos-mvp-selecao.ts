...
export const requisitosMvpSelecao: Requisito[] = [
  ...
  },
  {
    ...
    tipo_issue_recomendado: 'Task',
    criterios_aceite:
      'A decisão registra duas coisas distintas, cada uma sob a sua regra. A janela de solicitação de isenção é período do cronograma do processo, delimitado por datas, e não é interposição de recurso: não fica sujeita ao contrato de unidades de UNI-REQ-0080 nem depende de calendário ou algoritmo de contagem. O prazo de recurso contra o indeferimento, esse sim, é interposição, e a sua unidade e convenção de contagem seguem UNI-REQ-0080. Ambas registradas com justificativa, atualizando UNI-REQ-0104 antes da implementação.',
  ...
  },
  {
    ...
    titulo: 'Localidade e fuso horário que regem a contagem de prazos do processo seletivo (RN13)',
    enunciado:
      'Todo Processo Seletivo declara, na sua configuração, a localidade cujo calendário rege a contagem de prazos do certame, como a referência de cidade já canônica no sistema: código IBGE de sete dígitos, nome e UF — o mesmo trio que Campus e Local de Oferta persistem e validam, com prefixo do código coerente com a UF informada e nome não vazio, em que o **código IBGE é o único valor normativo**: é dele que se derivam o município cujos feriados locais se aplicam e a UF cujos feriados estaduais se aplicam, porque o prefixo do código determina a unidade federativa. A configuração declara também, como campo próprio e igualmente obrigatório, o **fuso horário** em que o dia civil começa e termina: informado por quem cadastra o processo, nunca fixado no sistema nem deduzido da cidade por tabela interna — deduzi-lo seria repor a constante num lugar menos visível, e é a regra de horário de verão, que muda por decreto, quem determina o valor correto. O nome e a UF persistidos são cache de exibição e não entram em cálculo nenhum — divergirem do município que o código designa é defeito de exibição, corrigível atualizando o cache, e jamais altera quais feriados incidem. O nome não é conferido contra o código, e não precisa ser: se divergirem, a versão exibe um município e conta pelos feriados do que o código designa — rótulo errado, prazo certo. Prometer coerência nominal seria prometer uma verificação que não existe. Os dois campos são sempre obrigatórios, qualquer que seja a unidade dos prazos configurados: nenhum admite ausência, e processo sem localidade ou sem fuso não é publicável. Não são condicionais porque todo prazo de interposição — inclusive em horas — passou a depender do calendário desde a primeira publicação (UNI-REQ-0080), de modo que não existe processo publicável em que localidade ou fuso sejam dispensáveis. O sistema não atribui nenhum dos dois por omissão: não os deriva da unidade administradora, do campus de oferta nem da sede da instituição, e não pré-seleciona valor. Um processo com ofertas em municípios diferentes tem uma única localidade regente e um único fuso, declarados no edital. Localidade e fuso são congelados em toda versão publicada; nas fases configuradas para aceitar recurso, UNI-REQ-0080 congela também o identificador do calendário e o nome e a versão do algoritmo de contagem declarado. Ambos são dados de configuração editáveis enquanto o processo está em rascunho — obrigatórios para publicar, não para criar.',
    grupo: 'negocio',
    tipo: 'regra_negocio',
    nivel: 'regra',
    parent_id: 'UNI-REQ-0014',
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'Publicar processo sem localidade, ou sem fuso horário, é recusado com erro nomeado — um por campo ausente —, qualquer que seja a unidade dos prazos configurados; a mesma recusa vale nas três transições que geram versão — publicação inicial, retificação em ato único e fechamento de sessão editorial —, nunca só na interface. O trio é validado quanto à forma e à coerência interna — sete dígitos, prefixo de código compatível com a UF informada, nome presente —, do mesmo modo que o sistema já valida a cidade de Campus e Local de Oferta; a escolha do município parte da lista apresentada ao operador, e este requisito não promete verificação de existência no momento da gravação — a cidade é dado de outro serviço, e o sistema não mantém cadastro próprio para cruzar. A consequência fica declarada em vez de silenciosa: um código bem formado que não designe município real é gravável, e o defeito só aparece quando os feriados municipais forem resolvidos, tornando a versão irreproduzível. Fechar essa porta exige verificar a existência do código junto ao serviço que detém o cadastro, no gate de publicação — mesma etapa em que a cobertura do calendário é aferida, e mesmo trabalho. Como o nome não é conferido contra o código, a coerência do cálculo não pode depender dele: o município e a UF que governam a contagem são derivados do código IBGE, nunca lidos dos campos informados, de modo que um nome divergente produz rótulo errado e nunca prazo errado. O fuso é aceito como identificador de zona horária da base IANA declarado por quem configura, e é validado quanto ao reconhecimento: identificador ausente e identificador não reconhecido são recusas distintas e nomeadas, porque congelar uma zona inexistente produziria versão publicada cuja âncora não converte para dia civil algum — prazo irreprodutível numa versão imutável. O sistema não deduz o fuso da cidade, da UF nem de qualquer tabela interna, e não oferece valor pré-selecionado. Toda versão publicada carrega localidade e fuso, de modo que uma retificação que mude a unidade de um prazo nunca precisa completar dado da versão anterior. Dois processos com localidades cujo conjunto de dias não úteis aplicáveis difira por ao menos um feriado situado dentro da janela produzem fins diferentes para a mesma âncora e o mesmo prazo; localidades que não difiram dentro da janela produzem o mesmo fim, e isso não é falha. Dois processos com o mesmo prazo e a mesma âncora em instante absoluto, mas com fusos declarados diferentes, podem atribuir esse instante a dias civis distintos e produzir fins distintos. Alterar localidade ou fuso em retificação não altera prazo de recurso já aberto sob a versão anterior.',
    verificacao:
      'Teste de contrato: publicação recusada sem localidade e, em erro distinto, recusada sem fuso, em processo com prazos em horas e em dias úteis; ambas as recusas presentes nas três transições que geram versão; nenhum dos dois campos pré-preenchido ou deduzido de cidade, UF ou unidade administradora; identificador de fuso não reconhecido pela base IANA recusado com erro distinto do de fuso ausente; trio incoerente (prefixo × UF, código fora de sete dígitos, nome vazio) recusado; localidade e fuso presentes e imutáveis na versão publicada, com retificação de qualquer um dos dois não alterando prazo já aberto sob a versão anterior; duas localidades que difiram por um feriado dentro da janela produzindo fins distintos para a mesma âncora e o mesmo prazo; uma referência cujo nome persistido divirja do município designado pelo código produzindo o mesmo fim que a referência correta, provando que o cálculo não lê o cache de exibição; e dois processos idênticos salvo pelo fuso declarado, com a mesma âncora absoluta, produzindo fins distintos quando o instante cai em dias civis diferentes (planejado).',
    pagina_developers: '/produto/regras-negocio/',
    owner: 'Tech Lead',
  },
  {
  ...
  },
  {
    ...
    enunciado:
      'A configuração do processo congela, por fase configurada para aceitar recurso, o prazo de interposição — como um par valor-unidade, com a convenção de contagem explícita, nunca um valor numérico sozinho, pois o mesmo número conta prazos diferentes conforme a convenção — referente à ÚNICA interposição que o Uni+ conduz (a primeira instância; UNI-REQ-0081), nunca à instância superior, que é externa e não tem interposição própria dentro do sistema. O prazo de interposição corre exclusivamente em dia útil: é o prazo que fecha a porta do candidato, e tempo que passa quando ele não tem como agir não pode consumir a janela. Só duas unidades são declaráveis — dias úteis, em que cada dia não útil é pulado, e horas, em que apenas as horas situadas em dia útil avançam o relógio: uma janela de 48 horas aberta numa sexta às 18h fecha na terça às 18h quando o fim de semana é não útil. Dia corrido é recusado nesta convenção, porque declará-lo é aceitar que a janela encolha ao calhar de cair em feriado. Dia útil é o dia civil inteiro no fuso declarado na configuração do processo (UNI-REQ-0111), tomado como o intervalo fechado no início e aberto no fim — o requisito não fixa zona horária alguma nem a deduz de outro campo, pela mesma razão que não fixa município: quem configura informa, e qualquer zona escrita aqui, ou derivada por tabela interna, envelheceria à primeira mudança de regra de horário de verão; feriado é sempre o dia civil completo, nunca meio expediente — esta convenção não é horário comercial, e uma janela em horas atravessa a madrugada de um dia útil normalmente. O valor declarado em dias úteis é inteiro: fração de dia útil é recusada, porque não tem leitura unívoca. Como a âncora raramente cai à meia-noite, "um dia útil" a partir de uma sexta às 18h admite mais de uma leitura — avançar uma data útil, consumir vinte e quatro horas situadas em dia útil, ou excluir o dia do começo e contar o seguinte por inteiro —, e elas fecham a janela em instantes diferentes. Qual delas vale **não é decisão deste requisito nem do sistema: é o que distingue um algoritmo de contagem de outro**. Os algoritmos são cadastrados com nome e versão, cada um definindo exatamente essa resolução, e a configuração do processo declara qual deles usa — do mesmo modo que declara localidade e fuso. Isso evita eleger uma leitura como universal antes de a base legal estar confirmada (UNI-REQ-0095) e permite que editais com convenções distintas coexistam. Nenhuma implementação escolhe em silêncio: sem algoritmo declarado não há publicação. Como a âncora pode cair em dia não útil, é preciso dizer o que acontece então: deslocar o início para o primeiro dia útil seguinte, ou contar a partir do instante original ignorando apenas os períodos não úteis. Como no caso da âncora fora da meia-noite, a resposta é do algoritmo de contagem declarado, não deste requisito. A âncora é o instante de publicação declarado pelo ato, e é **instante absoluto**: data, hora e o deslocamento em relação a UTC efetivamente aplicado, nunca uma hora local acompanhada apenas do nome do fuso. A distinção não é formalismo — numa transição de horário de verão, uma hora local pode não existir ou ocorrer duas vezes, e só o deslocamento congelado resolve qual das duas. Uma hora local declarada que caia em salto ou em repetição é recusada na entrada, com erro nomeado, em vez de ajustada por conta própria. A âncora também nunca é o instante em que o registro foi gravado no sistema, que é dado de auditoria e deslocaria a janela do candidato sempre que o registro atrasasse. O fuso declarado (UNI-REQ-0111) continua sendo o que delimita cada dia civil, inclusive nos dias de transição, que têm vinte e três ou vinte e cinco horas — a contagem não presume dia de vinte e quatro horas, porque avança sobre instantes situados em dia útil, não sobre blocos fixos. Por depender do calendário em todos os casos, o prazo de interposição fica sujeito ao mesmo regime condicional que já vale para a suspensividade: só é publicável quando existirem um calendário de dias úteis vigente cadastrado E um algoritmo de contagem declarado pelo processo, escolhido entre os nomeados no catálogo e identificado por nome e versão. Congelar nome e versão só reproduz o prazo se aquela versão for imutável: **a partir da primeira versão de configuração que a referencie, a entrada do catálogo vira fato e não pode ser alterada nem removida**; evolução é versão nova, que coexiste com a anterior. Ao contrário do calendário, cuja reprodutibilidade se resolveria copiando os dias por valor, o algoritmo é lógica executável e não há o que copiar para dentro do snapshot — a imutabilidade da versão referenciada é a única garantia possível, e por isso é condição do próprio congelamento. Enquanto nenhum algoritmo estiver disponível para declaração, a publicação é recusada, nunca aproximada em silêncio. Junto com ele, congela, por instância (primeira e segunda) nessa mesma fase, o efeito suspensivo como um par valor-unidade com convenção de contagem — cuja ausência (nulo) significa que a pendência não bloqueia; aqui dias úteis é aceito, mas só quando há um calendário de dias úteis vigente cadastrado E um algoritmo de contagem declarado pelo processo (é ele que define como um instante-âncora em dia não útil se resolve — UNI-REQ-0081), e o snapshot congela o identificador dos dois, a localidade e o fuso que o processo declara (UNI-REQ-0111) — a localidade determina quais feriados de abrangência estadual e municipal se aplicam, e o fuso, onde cada dia civil começa e termina; sem os dois, um mesmo calendário produz janelas diferentes conforme quem calcula, e nenhum dos quatro pode ser recalculado depois — não só a convenção "dias úteis" — porque tanto o calendário quanto o algoritmo escolhido podem ganhar versão nova depois da publicação, e sem prendê-los à versão congelada um recurso já em curso poderia fechar num instante diferente do que a garantia de UNI-REQ-0093 promete. Enquanto nenhum algoritmo de contagem estiver disponível para declaração — hoje nenhum está —, dias úteis na suspensividade é sempre recusado na publicação, pelo mesmo motivo que já recusa sem calendário vigente: nunca aproximar em silêncio. O escopo do bloqueio e a irreversibilidade do ato não são desses campos: o escopo deriva do objeto de cada recurso (UNI-REQ-0081) — erga omnes trava o processo inteiro, individual trava só o alcance afetado —; a irreversibilidade é classificação do TIPO de ato (por exemplo, convocação sempre consome vaga escassa), não um atributo configurável por edital nem por fase. As duas são regras estruturais, não configuração que uma retificação possa alterar — por isso não entram no snapshot. Os campos que entram — o prazo de interposição e o par de suspensividade por instância, mais os quatro dados que permitem calculá-los: identificador do calendário, nome e versão do algoritmo de contagem declarado, localidade e fuso — vão pro snapshot imutável na publicação e não podem ser adiados: o motor que os avalia vem depois (UNI-REQ-0081), mas a informação que ele exigirá precisa estar congelada desde já (RN08) — inclusive para processos publicados antes de o motor existir, já que a versão publicada é imutável e não há como completá-la depois (UNI-REQ-0093). Uma fase que não é configurada para aceitar recurso simplesmente não tem esses campos — isso não invalida a publicação (UNI-REQ-0094).',
    grupo: 'negocio',
    tipo: 'regra_negocio',
    nivel: 'regra',
    parent_id: 'UNI-REQ-0014',
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'O snapshot congelado carrega, por fase configurada para aceitar recurso, o prazo de interposição (valor, unidade e convenção de contagem, só dias úteis em valor inteiro ou horas) referente só à interposição que o Uni+ conduz — nunca um prazo de interposição para a instância superior, que não tem essa etapa dentro do sistema (UNI-REQ-0081) e por isso não precisa desse campo. A mesma fase carrega também, por instância (primeira e segunda), o par de suspensividade (valor, unidade e convenção de contagem — corridos, horas ou úteis —, ou nulo para não bloquear); a contagem do prazo de interposição ignora, nas duas unidades, os instantes de dia não útil segundo a versão do calendário cujo identificador foi congelado no snapshot, com o dia civil delimitado no fuso declarado na configuração do processo; dependem do calendário exatamente dois casos, e nenhum outro: todo prazo de interposição, seja em dias úteis, seja em horas, e a suspensividade em dias úteis — suspensividade em horas ou em dias corridos não depende, porque conta todos os dias sem distinguir úteis de não úteis. Nesses dois casos o snapshot carrega também o identificador da versão do calendário, o nome e a versão do algoritmo de contagem declarado, a localidade e o fuso — todos vigentes na publicação, nenhum dos quatro recalculado depois. Um processo com uma fase configurada para aceitar recurso mas publicado sem o prazo de interposição, sem convenção de contagem em qualquer um dos pares congelados, com valor zero ou negativo em qualquer um dos dois, ou com prazo de interposição (em qualquer unidade) ou suspensividade em dias úteis sem identificador de calendário vigente ou sem algoritmo de contagem declarado (hoje sempre o caso, pois nenhum algoritmo está disponível para declaração), é rejeitado, pois a versão é imutável e o gate futuro não teria como reconstruí-los; um prazo de interposição em dias corridos, ou em fração de dia útil, é sempre rejeitado; uma fase que não é configurada para aceitar recurso não precisa desses campos, e sua ausência nesse caso não é falha de publicação. Nem escopo do bloqueio nem irreversibilidade do ato entram no snapshot por fase: o primeiro é resolvido do objeto de cada recurso, o segundo do cadastro do tipo de ato — nenhum dos dois é configuração por edital. A configuração é a mesma para Seleção e Ingresso; a diferença de bloqueio decorre só dos valores congelados, nunca de ramificação por módulo em código.',
    verificacao: 'Teste de contrato do snapshot (prazo de interposição e par de suspensividade presentes e imutáveis após publicação para fase configurada com recurso, com convenção de contagem sempre explícita; valor zero ou negativo recusado nos dois; identificador de calendário, nome e versão do algoritmo declarado, mais a localidade e o fuso, congelados junto com o prazo de interposição em qualquer unidade e a suspensividade em dias úteis, imunes a correção posterior de qualquer um dos quatro; dois processos com fusos declarados distintos, com a mesma âncora em instante absoluto, atribuindo o instante a dias civis diferentes e produzindo fins diferentes; publicação recusada quando o fuso não foi declarado; âncora declarada em hora local que não existe no fuso, por salto de horário de verão, recusada com erro nomeado, e o mesmo para hora local repetida na volta, em vez de resolvida por escolha implícita de deslocamento; janela que atravessa uma transição fechando pelo mesmo total de horas úteis, ainda que o dia civil da transição tenha vinte e três ou vinte e cinco horas; dia corrido e fração de dia útil recusados no prazo de interposição sempre, e prazo de interposição em qualquer unidade e suspensividade em dias úteis recusados sempre que faltar calendário vigente ou algoritmo declarado, e suspensividade em horas ou dias corridos aceita sem eles; dois processos que declarem algoritmos diferentes, com a mesma âncora, o mesmo prazo e o mesmo calendário, podem fechar em instantes diferentes, e cada um é reproduzível pela sua versão; alterar ou remover uma versão de algoritmo já referenciada por versão de configuração publicada é recusado, e publicar uma versão nova do mesmo algoritmo não altera o fim de recurso aberto sob a anterior; contraprovas numéricas da contagem em horas sobre dia útil: 48 horas ancoradas numa sexta às 18h, com sábado e domingo não úteis, fecham na terça às 18h; 24 horas ancoradas às 00:00 de uma segunda útil fecham às 00:00 de terça; 24 horas ancoradas às 00:00 de um domingo não avançam no domingo e, sendo a segunda útil, fecham também às 00:00 de terça; 30 horas ancoradas numa segunda útil às 00:00, com terça feriado e quarta útil, fecham na quarta às 06:00; e 24 horas nessa mesma âncora fecham às 00:00 de terça, primeiro instante do feriado, porque o saldo zerou antes de o feriado precisar ser pulado; a resolução de âncora não situada à meia-noite na unidade dias úteis, e a de âncora situada em dia não útil fora da meia-noite, são contraprovas obrigatórias de cada algoritmo do catálogo, com resultado declarado por algoritmo para âncora em sexta às 18h e para âncora em domingo às 18h — hoje não exercitável, porque nenhum prazo de interposição é publicável sem algoritmo declarado; instância superior publica sem prazo de interposição próprio; fase sem recurso publica sem eles) (planejado).',
    pagina_developers: '/produto/regras-negocio/',
    owner: 'Tech Lead',
  },
  {
