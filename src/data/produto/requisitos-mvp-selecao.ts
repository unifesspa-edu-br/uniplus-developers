...
export const requisitosMvpSelecao: Requisito[] = [
  ...
  },
  {
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'Cada campo/fato declara pré-condição (predicado sobre fatos anteriores) no vocabulário e operadores do gatilho; o grafo é acíclico e ciclo é recusado no cadastro; exclusão indígena versus quilombola é pré-condição, não código; o gate de escola pública é pré-condição de todos os campos/opt-ins de subcota de escola pública, exceto a dimensão PcD; o campo com pré-condição falsa não é apresentado; o grafo congela na publicação.',
    verificacao:
  ...
  },
  {
    ...
    enunciado:
      'A MODALIDADE de concorrência é um fato DERIVADO multivalorado — o conjunto de modalidades a que o candidato concorre — computado pela regra de derivação a partir dos pares elegibilidade e opt-in, e nunca declarado diretamente. O domínio canônico é {AC, AC_PcD, LI_EP, LB_EP, LI_PPI, LB_PPI, LI_Q, LB_Q, LI_PcD, LB_PcD} (o V do corpus é alias canonicalizado para AC_PcD). As dependências da regra são {CONCORRER_PCD, ESCOLA_PUBLICA, CONCORRER_EP, CONCORRER_PPI, CONCORRER_Q, CONCORRER_RENDA}. A composição da Lei de Cotas é configuração congelada, não ramificação por tipo de processo em código: AC como âncora (regra de DNF vazio); concorrência dupla (o cotista também concorre em AC); as modalidades LB_*, com critério de renda per capita inferior a um salário mínimo, também concorrem às respectivas LI_*; as subcotas de escola pública exigem ESCOLA_PUBLICA; a pessoa com deficiência com opt-in concorre à AC_PcD mesmo sem escola pública; indígena não gera cota quilombola. A regra é o conjunto de regras {quando, contribui} R0 a R9 (cada LB_* repete os átomos da LI_* irmã mais o átomo de renda), modelo todo-positivo cujo resultado é a UNIÃO das contribuições ativas, restrito no último passo às modalidades ofertadas pelo processo. A configuração concreta R0–R9 e a matriz completa de combinações alcançáveis são normativas (não fixtures exemplificativas) e correspondem ao formulário real de definição de cotas, com as correções editoriais assumidas do corpus (referências cruzadas deslocadas 2.2/3.1/3.2 lidas pelo bloco correto; LI_PPI/LB_PPI presentes na lista; AC_PcD canônico com V como alias).',
    grupo: 'funcional',
    tipo: 'requisito_funcional',
    nivel: 'requisito',
    parent_id: 'UNI-REQ-0016',
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'MODALIDADE é fato DERIVADO multivalorado (cadastro como DECLARADO é recusado) com domínio canônico {AC, AC_PcD, LI_EP, LB_EP, LI_PPI, LB_PPI, LI_Q, LB_Q, LI_PcD, LB_PcD} e dependências {CONCORRER_PCD, ESCOLA_PUBLICA, CONCORRER_EP, CONCORRER_PPI, CONCORRER_Q, CONCORRER_RENDA}; as regras R0–R9 (AC por DNF vazio; LB_* = átomos da LI_* irmã mais CONCORRER_RENDA) e todas as linhas da matriz normativa de combinações alcançáveis são critérios de aceite, não fixtures; alias V canonicalizado para AC_PcD; correções editoriais do corpus assumidas (refs 2.2/3.1/3.2 deslocadas; LI_PPI/LB_PPI presentes); união todo-positivo restrita às modalidades ofertadas como último passo; verde de ponta a ponta (não-EP resolve a {AC}/{AC, AC_PcD}; renda NÃO_APLICÁVEL mantém só as LI_*; PPI+Q simultâneos e múltiplas dimensões; expansão LB_* para LI_*).',
    verificacao:
      'Teste automatizado / matriz de aceitação (cada combinação de respostas para o conjunto de modalidades esperado, incluindo R0–R9 e todas as linhas da matriz; MODALIDADE não declarável; alias V para AC_PcD; restrição pela oferta) (planejado).',
    pagina_developers: '/produto/requisitos/',
    owner: 'Equipe backend; Equipe frontend',
  },
  {
  ...
  },
  {
    ...
    enunciado:
      'O mesmo snapshot conjunto e autossuficiente de UNI-REQ-0071 é estendido, na publicação, para congelar de forma determinística o vocabulário e os domínios de todos os fatos, independentemente da origem — inclusive os fatos DERIVADOS e o domínio canônico de MODALIDADE usado para validar contribuições, canonicalizar o alias V para AC_PcD e intersectar a oferta —, além do grafo de dependência (as quatro arestas), das regras de derivação, da versão semântica do interpretador, da ordem topológica total e do conjunto de modalidades ofertadas usado na interseção. A ordem topológica é total por uma chave canônica: todo nó tem código estável obrigatório único no seu tipo e escopo; o escopo é o código canônico do agregado; a identidade canônica idCanonico é tipoDeNo/escopo/codigo em UTF-8 NFC com comparação ordinal, nunca identificador de runtime; a chave total é (faseTopologica, ordem, idCanonico). A canonicalização para o hash usa as chaves e ordens fechadas da spec para todos os conjuntos — nós pela chave total, arestas por (tipoAresta, origem, destino), dependências, valores de EM/NAO_EM ordenados sem duplicatas, átomos por (fato, operador, valorCanônico), cláusulas e DNF por sequência ordenada, regras por (contribui, dnf) e oferta —, com aliases canonicalizados antes de validar, persistir e hashear. A schema_version congelada seleciona um perfil de JSON canônico byte a byte que fixa chaves ordenadas, NFC, ausência de espaço insignificante, tabela de escapes e de caracteres de controle, ausência distinta de nulo (null em campo opcional não anulável é recusado) e forma numérica canônica, de modo que o mesmo conteúdo semântico produz os mesmos bytes e o mesmo hash em qualquer implementação conforme. A publicação recusa código de fato duplicado, mais de um binding de derivação ativo por fato derivado, referência a nó fora do snapshot, ciclo no grafo conjunto, dependência não declarada, código de modalidade não canônico e colisão de idCanonico. Alterar o grafo, a regra ou o domínio vivo não afeta editais publicados.',
    grupo: 'funcional',
    tipo: 'requisito_funcional',
    nivel: 'requisito',
    parent_id: 'UNI-REQ-0019',
    ...
    tipo_issue_recomendado: 'Story',
    criterios_aceite:
      'Estende o snapshot autossuficiente de UNI-REQ-0071; congela vocabulário/domínios de todos os fatos (inclusive DERIVADOS e o domínio canônico de MODALIDADE), grafo de quatro arestas, regras de derivação, versão semântica, ordem topológica total e modalidades ofertadas; todo nó com código estável e idCanonico = tipoDeNo/escopo/codigo (escopo = código do agregado; UTF-8 NFC ordinal; não runtime); chave total (faseTopologica, ordem, idCanonico); canonicalização com as chaves fechadas da spec (arestas por (tipoAresta, origem, destino); átomo (fato, operador, valorCanônico); EM/NAO_EM ordenado sem duplicatas; cláusula/DNF ordenados; regra (contribui, dnf)); alias V canonicalizado para AC_PcD antes de validar/persistir/hashear; schema_version seleciona o perfil JSON canônico byte a byte (chaves ordenadas, NFC, escapes e controles fixos, ausência distinta de null, null em opcional não anulável recusado, número canônico); publicação recusa código duplicado, mais de um binding ativo por derivado, referência fora do snapshot, ciclo, dependência não declarada, código não canônico e colisão de idCanonico; mudança no vivo não altera edital publicado.',
    verificacao:
      'Teste automatizado / golden test (hash idêntico a permutação de cada conjunto — nós, arestas inclusive mesmo par com tipoAresta distinto, dependências, EM/NAO_EM, átomos, cláusulas, regras, oferta, V versus AC_PcD, Unicode escapado versus literal, caracteres de controle, números equivalentes; recusa de null inválido e de colisão de idCanonico; imutabilidade a mudanças de catálogo) (planejado).',
    pagina_developers: '/produto/requisitos/',
    owner: 'Equipe backend; Equipe frontend',
  },
];
