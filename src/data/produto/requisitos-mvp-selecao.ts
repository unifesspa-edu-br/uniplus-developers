...
export const requisitosMvpSelecao: Requisito[] = [
  ...
  },
  {
    ...
    tipo_issue_recomendado: 'Task',
    criterios_aceite:
      'A janela de suspensividade de um recurso é resolvida pela versão de configuração vigente quando o ato recorrido foi publicado, nunca pela vigente no instante da interposição nem no julgamento; uma retificação publicada entre a publicação do ato recorrido e a interposição não altera o prazo nem o escopo do bloqueio de um recurso que venha a ser interposto contra aquele ato; um recurso contra um ato publicado já sob a regra retificada é regido pela regra nova, sem herdar a proteção de atos anteriores; uma fase que perde a configuração de recurso numa retificação recusa nominalmente a tentativa de interposição contra ela, nunca permanece silenciosamente sem bloqueio.',
    verificacao:
      'Teste automatizado (recurso interposto após retificação continua sob a janela do ato recorrido original; recurso contra ato publicado já sob a regra nova não herda a proteção de recurso anterior; retificação que remove a configuração de recurso de uma fase recusa a interposição nominalmente, sem se tornar invisível ao bloqueio) (planejado).',
    pagina_developers: '/produto/regras-negocio/',
    owner: 'Tech Lead',
  },
  {
